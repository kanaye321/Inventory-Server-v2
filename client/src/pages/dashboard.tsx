...
export default function Dashboard() {
  ...
  const { data: activities } = useQuery({
    ...
    staleTime: 1000 * 2, // Cache for 2 seconds
  ...
  // Recent IT operations — show all latest activity, sorted newest first
                ...
                {criticalActivities.length > 0 ? (
                  criticalActivities.map((activity: any, index: number) => {
                    const actionColors: Record<string, string> = {
                      create: "bg-green-100", update: "bg-blue-100", delete: "bg-red-100",
                      checkout: "bg-purple-100", checkin: "bg-teal-100", maintenance: "bg-yellow-100",
                    };
                    const iconColor: Record<string, string> = {
                      create: "text-green-600", update: "text-blue-600", delete: "text-red-600",
                      checkout: "text-purple-600", checkin: "text-teal-600", maintenance: "text-yellow-600",
                    };
                    const bg = actionColors[activity.action] ?? "bg-gray-100";
                    const ic = iconColor[activity.action] ?? "text-gray-600";
                    ...
                    return (
                      <div key={index} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div className={`w-8 h-8 ${bg} rounded-full flex items-center justify-center flex-shrink-0`}>
                          <Activity className={`h-4 w-4 ${ic}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 truncate">
                            <span className={`inline-block text-xs font-semibold mr-1.5 ${ic}`}>[{actionLabel}]</span>
                            {activity.notes || `${actionLabel} on ${activity.itemType}`}
                          </p>
                          <p className="text-xs text-gray-500">
                            {activity.itemType} · {format(new Date(activity.timestamp), 'MMM dd, HH:mm')}
                          </p>
                        </div>
                      </div>
                    );
